...
Entity_Mamorb::Entity_Mamorb(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Mamorb, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x07D6);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Mamorb::Think()
{
    anim->Tick();
}

void Entity_Mamorb::Render(Renderer* renderer)
{
