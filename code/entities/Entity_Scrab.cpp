...
Entity_Scrab::Entity_Scrab(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Scrab, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x02BC);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Scrab::Think()
{
    anim->Tick();
}

void Entity_Scrab::Render(Renderer* renderer)
{
