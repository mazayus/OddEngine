...
Entity_Bomb::Entity_Bomb(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Bomb, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x040D);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Bomb::Think()
{
    anim->Tick();
}

void Entity_Bomb::Render(Renderer* renderer)
{
