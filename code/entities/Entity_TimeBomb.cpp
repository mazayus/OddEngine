...
Entity_TimeBomb::Entity_TimeBomb(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_TimeBomb, data, size)
{
    x = (x1 + x2) / 2;
    y = (y1 + y2) / 2;

    const AnimSet* anim_set = level->FindAnimSet(0x03ED);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_TimeBomb::Think()
{
    anim->Tick();
}

void Entity_TimeBomb::Render(Renderer* renderer)
{
