...
Entity_Slog::Entity_Slog(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Slog, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x023A);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Slog::Think()
{
    anim->Tick();
}

void Entity_Slog::Render(Renderer* renderer)
{
