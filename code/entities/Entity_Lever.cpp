...
Entity_Lever::Entity_Lever(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Lever, data, size)
{
    x = (x1 + x2) / 2;
    y = y1;

    const AnimSet* anim_set = level->FindAnimSet(0x03F1);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Lever::Think()
{
    anim->Tick();
}

void Entity_Lever::Render(Renderer* renderer)
{
