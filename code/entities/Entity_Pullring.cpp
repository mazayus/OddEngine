...
Entity_Pullring::Entity_Pullring(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Pullring, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x03F6);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Pullring::Think()
{
    anim->Tick();
}

void Entity_Pullring::Render(Renderer* renderer)
{
