...
Entity_Slig::Entity_Slig(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Slig, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x019C);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Slig::Think()
{
    anim->Tick();
}

void Entity_Slig::Render(Renderer* renderer)
{
