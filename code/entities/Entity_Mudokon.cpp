...
Entity_Mudokon::Entity_Mudokon(const OddLevel* level, const void* data, unsigned int size) :
    Entity(Entity::Type_Mudokon, data, size)
{
    x = (x1 + x2) / 2;
    y = y2;

    const AnimSet* anim_set = level->FindAnimSet(0x01FF);
    anim.reset(new Anim(anim_set, 0, 0));
}

void Entity_Mudokon::Think()
{
    anim->Tick();
}

void Entity_Mudokon::Render(Renderer* renderer)
{
